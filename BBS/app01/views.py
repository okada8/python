...
class Home(View):
    def get(self,requwst):
        ...
        return render(requwst,'home.html',locals())
...
#         content = request.POST.get('content')
#
#         # 只所以要使用bs4模块,就是为了解决xss攻击以及html文档文本值的截取
#
#         # 将html文本交给BeautifulSoup生成soup对象
#         soup = BeautifulSoup(content, 'html.parser')  # 第二个参数是解析器类型,有多种可供选择,我们只有最简单的
#         # print(soup.find_all())
#         tags = soup.find_all()
#         for tag in tags:
#             # 获取html文档所有的标签名tag.name
#             if tag.name == 'script':
#                 # 如果是script标签,直接删除
#                 tag.decompose()  # 调用自带的删除方法删除指定的标签
#         # # 文章摘要
#         # desc = content[0:150]
#         # print(soup.text)  # 获取html文档所有的文本值
#         desc = soup.text[0:150]
#         # # 添加文章
#         models.Article.objects.create(title=title, content=str(soup), desc=desc, blog=request.user.blog)
#         # 用str(soup)传入数据库,
