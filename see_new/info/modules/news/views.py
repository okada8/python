...
from info.models import Comment,News,CommentLike
...
def comment_like():
    ...
    if action == "add":
        commentss_like = CommentLike.query.filter(CommentLike.comment_id == comments_id, CommentLike.user_id == user.id).first()
        ...
        comment_likess=CommentLike.query.filter(CommentLike.user_id==user.id,CommentLike.comment_id==comment.id).first()
        if comment_likess:
